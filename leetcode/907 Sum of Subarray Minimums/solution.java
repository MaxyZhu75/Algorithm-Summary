	...
	public int sumSubarrayMins(int[] arr) { // arr[i]的辐射范围是一个连续数组，辐射范围内产生的所有子数组最小值都为arr[i]！！！
        ...
        for (int i=0; i<n; i++) { // 步骤一：从0到n-1遍历，利用单调栈向左找到第一个小于等于arr[i]数，即辐射范围的左边界！！！
            while (!myStack.isEmpty() && arr[myStack.peek()] > arr[i]) {
                myStack.pop(); // 不小于等于arr[i]则弹出栈！！！
            }
            if (myStack.isEmpty()) { // 向左未找到左边界则定为-1！！！
                left[i] = -1;
            } else {
                left[i] = myStack.peek(); // 存储左边界的值，不弹出！！！
            }
            myStack.push(i); // 每次循环将下标入栈，方便同时得到i和arr[i]！！！
        }

        myStack.clear(); // 清空栈内元素！！！
		
        for (int i = n - 1; i >= 0; i--) { // 步骤一：从n-1到0遍历，利用单调栈向右找到第一个严格小于arr[i]的数，即辐射范围的右边界！！！
            while (!myStack.isEmpty() && arr[myStack.peek()] >= arr[i]) { // 仅将右边界判断条件设置为严格小于，是因为当一个子数组中有两个最小值元素时，需要不重复且不遗漏地统计每一个子数组！！！
                myStack.pop(); // 不严格小于arr[i]则弹出栈！！！
            }
            if (myStack.isEmpty()) { // 向右未找到右边界则定为n！！！
                right[i] = n;
            } else {
                right[i] = myStack.peek(); // 存储右边界的值，不弹出！！
            }
            myStack.push(i); // 每次循环将下标入栈，方便同时得到i和arr[i]！！！
        }
