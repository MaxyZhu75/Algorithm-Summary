 ...
 */
class Solution {
    public List<Integer> preorderTraversal(TreeNode root) { // Morris遍历在线性时间完成且空间复杂度为O(1)！！！
        List<Integer> result = new ArrayList<Integer>();
        if (root == null) return result; // 前序遍历：根左右！！！
        ...
        while (subtreeRoot != null) {
            if (subtreeRoot.left != null) { // case1：左边不为空时说明还未办手续，需要先对当前节点的左边办手续，总体上下潜！！！
                ...
                if (predecessor.right == null) { // 未连接：注意前序遍历需要先将当前subtreeRoot加入result；再将predecessor指向当前subtreeRoot后，subtreeRoot才继续往左边下潜！！！
