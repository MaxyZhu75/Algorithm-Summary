class Solution {
    public int[] topKFrequent(int[] nums, int k) { // 桶排序的经典实现！！！
        ...
        for (int i = buckets.length-1; i>=0 && topK.size()<k; i--) { // 由于buckets的索引表示频率，故此处使用倒叙遍历来找最大频率的k个元素！！！
            int available = k-topK.size(); // topK只需要k个元素，用available记录topK还需要添加多少个元素！！！
            if (buckets[i] == null) {
                continue;
            } else if (buckets[i].size() <= available) {
                topK.addAll(buckets[i]); // case1：ArrayList.addAll()；添加集合中的所有元素到topK中！！！
            } else {
                topK.addAll(buckets[i].subList(0, available-1)); // case2：ArrayList.subList()；只添加available个元素，使topK大小刚好为k个元素！！！
            }
        }
